...
export function countsByPos(list) {
...
export function canAdd(player) {
  ...
  if (state.team.some((t) => t.id === player.id))
    return [false, "השחקן כבר בקבוצה"];
  const caps = activeRule();
  const used = countsByPos(state.team)[player.pos] || 0;
  const cap = caps[player.pos] ?? 0;
